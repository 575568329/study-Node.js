...
**最后更新**:2026-08-27（前端复习线 ⑰ Next.js —— React 全景收官 🎉；同日 Java 线 Day 31 见下方）

## 2026-08-27 学习记录（前端复习线）

### 课前小测（5 题）：3 G + 1 H + 1 A

- ✅ 状态下放 / get vs find / "Fiber"第 4 次终于固化（移出顽固名单）
- ❌ z.infer 第 3 次 A → 新记法：三级跳推理链（schema 值→typeof 升类型→infer 提形状）
- ⚠️ 受控判断标准钉死：值存哪、谁说了算

### 学习成果

**⑰ Next.js（概念级收官，React 17 站全景 ✅）**

- CSR 三痛点：首屏慢 / SEO 拿到空 HTML / 弱网慢
- 渲染模式选型锚点："这页内容多久变一次？"（每次变 SSR / 不变 SSG / 偶尔变 ISR）
- RSC：默认服务端跑（零 JS+可连 DB），`'use client'` 回浏览器；全局状态库领地收缩到 Client 边界

### 🎉 里程碑

**React 复习全景 17 站收官**（08-05→08-27，22 天）；复习线五大主题（JS/TS/Vue3/CSS/Node.js/React）全部完成，转入 FSRS 维护模式。

### 遗留问题 / 下次计划

- 复习线维护模式：跟 Java 线共用课前小测调度（FSRS 队列持续清账）
- 可能延伸：05-React讲稿沉淀 / Next.js 实战（resume-web 改造候选）
- 近期到期关注：Zod 推理链（08-29）/ Router、axios、构建工具链（08-29）/ Next.js、受控非受控、自定义 Hook（08-30/31）
